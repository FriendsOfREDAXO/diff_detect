...
class HtmlOnepage
{
     ...
     * replace all images.
     */
    protected function dataUriPage(): void
    {
    ...
    protected function dataUriResources(
        $content,
    ): string {
    ...
    protected function linkCss()
    {
        $this->content = preg_replace_callback(
            '~(<link[^>]+?href=[\"\'])(.*?)([\"\'][^>]*>)~ism',
            function ($match) {
                if (!str_contains($match[1], 'rel="stylesheet"') && !str_contains($match[1], 'rel=\'stylesheet\'') && !str_contains($match[3], 'rel="stylesheet"') && !str_contains($match[3], 'rel=\'stylesheet\'')) {
                    return $match[0];
                }
    ...
    protected function linkJavaScript()
    {
        $this->content = preg_replace_callback(
